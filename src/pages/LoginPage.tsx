...
export default function LoginPage() {
  ...
  const { signInWithUsername, signInWithGoogle, user, loading: authLoading } = useAuth();
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();

  // Mostrar aviso si el usuario fue redirigido por cuenta inactiva
  useEffect(() => {
  ...
  // Redirigir al panel una vez que el contexto confirme la sesión activa
  useEffect(() => {
    if (user && !authLoading) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast.success('¡Bienvenido a CampusNOVA!');
      // La navegación ocurre automáticamente cuando el contexto actualiza `user`
    }
  };
        ...
        <Card className="shadow-hover">
          <CardHeader className="text-center pb-4">
            <div className="flex justify-center mb-4">
              <img src={LOGO_URL} alt="COTECNOVA" className="h-16 w-auto object-contain" />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex items-start gap-2">
                ...
                <Label htmlFor="terms" className="text-sm text-muted-foreground leading-relaxed cursor-pointer">
                  Acepto los{' '}
                  <span className="text-primary underline cursor-pointer">Términos y Condiciones</span>
                  {' '}y la{' '}
                  <span className="text-primary underline cursor-pointer">Política de Privacidad</span>
                </Label>
              </div>

              <Button type="submit" className="w-full" disabled={loading || googleLoading || authLoading}>
